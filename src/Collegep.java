...
public class Collegep{
    ...
    public static College findCollegeWithMaximumPincode(College[] college)
    {
      /*  int max = 0;
        ...
        return null;*/
        List<College> collegeList = Arrays.asList(college);

        // Sort the list in descending order by pincode
        collegeList.sort(Comparator.comparingInt(College::getPin).reversed());

        // Return the first element (has the maximum pincode) or null if the list is empty
        return collegeList.isEmpty() ? null : collegeList.get(0);
    }
