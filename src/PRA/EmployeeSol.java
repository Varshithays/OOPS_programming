...
class EmployeeG
{
    ...
    public EmployeeG(int id, String name, int age, char gender, double salary)
    {
        this.id = id;
        this.name = name;
        this.age = age;
...
{
    public static EmployeeG getEmployeeWithSecondLowestSalary(ArrayList<EmployeeG> employees, int n)
    {
        if(n<2)
        {
            return null;
        }
        employees.sort(Comparator.comparingDouble(EmployeeG::getSalary));    //comparator operator
        double firstLowestSalary = employees.get(2).getSalary();
        for (EmployeeG employee : employees)
        {
    ...
    public static int countEmployeesBasedOnAge(ArrayList<EmployeeG> employees, int reqage)
    {
        int count=0;
        for(EmployeeG employee:employees)
        {
    ...
    public static void main(String args[])
    {
        ...
        ArrayList<EmployeeG> Employees=new ArrayList<>();   //important to be noted
        for(int i=0; i<n; i++)
        {
            ...
            Employees.add(new EmployeeG(employeeId,employeeName,age,gender,salary));   //important to be noted
        }
        int reqage=sc.nextInt();
        EmployeeG sls= getEmployeeWithSecondLowestSalary(Employees,n);
